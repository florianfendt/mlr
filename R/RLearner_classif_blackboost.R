...
trainLearner.classif.blackboost = function(.learner, .task, .subset, .weights = NULL, mstop, nu, risk, teststat, testtype, mincriterion, maxdepth, ...) {
  ...
  if (!is.null(.weights))
    mboost::blackboost(f, data = .task[.subset, ], control = ctrl, tree_controls = tc, weights = .weights, ...)
  else
    mboost::blackboost(f, data = .task[.subset, ], control = ctrl, tree_controls = tc, ...)
}
...
predictLearner.classif.blackboost = function(.learner, .model, .newdata, ...) {
  ...
  if (.learner$predict.type == "prob") {
    ...
    colnames(y) = getTaskClassLevels(.model)
    y[, 1L] = p
    y[, 2L] = 1-p
    return(y)
  } else {
